...
⚠️ **重要**：此步驟會呼叫 AI API 為所有單字預生成選項，請確保已正確設定 API Key

1. 在 GAS 編輯器中，選擇函數 `initializeSpreadsheet`
2. 點擊執行 (▶)
3. 如果首次執行，會要求授權，按照提示授權即可
4. **等待完成** (約 10-30 秒，取決於單字數量和 API 速度)
5. 檢查執行記錄：
   - ✅ 應該看到類似 "已生成 Ubiquitous 的選項" 的訊息
   - ✅ 最後顯示 "Spreadsheet 初始化完成！成功生成 10/10 個單字的選項"
6. 回到 Google Sheets，檢查 **Vocabulary** 工作表的 **Options_Cache** 欄 (C 列)
   - 應該看到所有單字都有 JSON 格式的選項快取

**預生成的好處**:
- ✅ 每次答題時無需呼叫 API，加載速度快
- ✅ 大幅降低 API 成本 (Gemini 免費，OpenAI 節省 90%+ 費用)
- ✅ 穩定性更高，無需等待 API 回應
